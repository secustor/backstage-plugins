...
import { mcpWrapperPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [mcpWrapperPlugin],
    });

    await request(server).get('/api/mcp-wrapper/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/mcp-wrapper/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/mcp-wrapper/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/mcp-wrapper/todos/${createdTodoItem.id}`)
